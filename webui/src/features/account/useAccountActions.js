...
export function useAccountActions({ apiFetch, t, onMessage, onRefresh, config, fetchAccounts, resolveAccountIdentifier }) {
    ...
    const pauseAccount = async (identifier, pause) => {
        const accountID = String(identifier || '').trim()
        if (!accountID) {
            onMessage('error', t('accountManager.invalidIdentifier'))
            return
        ...
        try {
            ...
            const data = await res.json()
            if (!res.ok) {
                onMessage('error', data.detail || (pause ? t('accountActions.pauseFailed') : t('accountActions.resumeFailed')))
                return
            }
            onMessage('success', pause ? t('accountActions.pauseSuccess') : t('accountActions.resumeSuccess'))
            fetchAccounts()
            onRefresh()
        } catch (_err) {
            onMessage('error', t('messages.networkError'))
